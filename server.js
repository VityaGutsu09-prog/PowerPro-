...
async function generateBackground(flavor, contentType) {
  ...
  if (bgData.artifacts && bgData.artifacts[0]) return bgData.artifacts[0].base64;
  return null;
}
...
app.post('/api/image', async (req, res) => {
  try {
    ...
    if (productMatch) {
      ...
      if (!backgroundBase64) {
        console.log('First bg attempt failed, retrying...');
        ...
        if (simpleBgData.artifacts && simpleBgData.artifacts[0]) backgroundBase64 = simpleBgData.artifacts[0].base64;
      }
      res.json({ productImage: '/' + productMatch.file, backgroundBase64: backgroundBase64 || null, flavor: productMatch.flavor });
      return;
    }
